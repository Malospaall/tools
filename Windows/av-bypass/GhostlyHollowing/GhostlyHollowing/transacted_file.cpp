...
HANDLE make_transacted_section(wchar_t* dummy_name, BYTE* payladBuf, DWORD payloadSize)
{
    ...
    if (hTransaction == INVALID_HANDLE_VALUE) {
        DEBUG_ERR("Failed to create transaction!" << std::endl);
    ...
    if (hTransactedFile == INVALID_HANDLE_VALUE) {
        DEBUG_ERR("Failed to create transacted file: " << GetLastError() << std::endl);
        return INVALID_HANDLE_VALUE;
    }

    DWORD writtenLen = 0;
    if (!WriteFile(hTransactedFile, payladBuf, payloadSize, &writtenLen, NULL)) {
        DEBUG_ERR("Failed writing payload! Error: " << GetLastError() << std::endl);
        return INVALID_HANDLE_VALUE;
    }
    ...
    if (status != STATUS_SUCCESS) {
        DEBUG_ERR("NtCreateSection failed" << std::endl);
        return INVALID_HANDLE_VALUE;
    }
    ...
    if (RollbackTransaction(hTransaction) == FALSE) {
        DEBUG_ERR("RollbackTransaction failed: " << std::hex << GetLastError() << std::endl);
        return INVALID_HANDLE_VALUE;
    }
