...
{
#ifdef _DEBUG
    DEBUG_MSG("Writing new EP: " << std::hex << entry_point_va << std::endl);
#endif
#if defined(_WIN64)
    if (is32bit) {
...
ULONGLONG get_remote_peb_addr(PROCESS_INFORMATION &pi, bool is32bit)
{
    ...
    if (is32bit) {
        ...
        if (!Wow64GetThreadContext(pi.hThread, &context)) {
#ifdef _DEBUG
            DEBUG_MSG("Wow64 cannot get context!\n");
#endif
            return 0;
        }
...
#ifdef _DEBUG
        DEBUG_ERR("Cannot update remote EP!\n");
#endif
        return false;
    }
    return true;
...
bool set_new_image_base(BYTE* loaded_pe, PVOID load_base, PROCESS_INFORMATION& pi, bool is32bit)
{
    ...
    if (!remote_peb_addr) {
#ifdef _DEBUG
        DEBUG_ERR("Failed getting remote PEB address!\n");
#endif
        return false;
    }
    ...
    {
#ifdef _DEBUG
        DEBUG_ERR("Cannot update ImageBaseAddress!\n");
#endif
        return false;
        return false;
    }
