...
HANDLE open_file(wchar_t* filePath)
{
    ...
    if (!NT_SUCCESS(stat)) {
        DEBUG_MSG("Failed to open, status: " << std::hex << stat << std::endl);
        return INVALID_HANDLE_VALUE;
    }
    DEBUG_MSG("[+] Created temp file: " << filePath << "\n");
    return file;
}
...
HANDLE make_section_from_delete_pending_file(wchar_t* filePath, BYTE* payladBuf, DWORD payloadSize)
{
    ...
    if (!hDelFile || hDelFile == INVALID_HANDLE_VALUE) {
        DEBUG_ERR("Failed to create file" << std::dec << GetLastError() << std::endl);
        return INVALID_HANDLE_VALUE;
    }
    NTSTATUS status = 0;
    IO_STATUS_BLOCK status_block = { 0 };
    ...
    if (!NT_SUCCESS(status)) {
        DEBUG_MSG("Setting information failed: " << std::hex << status << "\n");
        return INVALID_HANDLE_VALUE;
    }
    DEBUG_MSG("[+] Information set\n");
    ...
    if (!NT_SUCCESS(status)) {
        ...
        DEBUG_ERR("Failed writing payload! Error: " << std::hex << err << std::endl);
        return INVALID_HANDLE_VALUE;
    }
    DEBUG_MSG("[+] Written!\n");
    ...
    if (status != STATUS_SUCCESS) {
        DEBUG_ERR("NtCreateSection failed: " << std::hex << status << std::endl);
        return INVALID_HANDLE_VALUE;
    }
    NtClose(hDelFile);
    hDelFile = nullptr;
